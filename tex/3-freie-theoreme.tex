...
um eine \textit{polymorphe Funktion}.

Die Instanziierung dieser Funktion auf einen konkreten Typ wird im Folgenden beschrieben, indem man den Typ in den Index
schreibt. $f_{Integer}$ ist dann beispielsweise die Funktion $f$, konkretisiert auf den Typ Integer, d.h. es wird Integer
...
$Bool \rightarrow Bool$, sie hat also die Signatur $[Bool \rightarrow Bool] \rightarrow [Bool \rightarrow Bool]$.

Hat die Funktion mehrere verschiedene Typvariablen, so werden die einzelnen Typen
in der Reihenfolge des Auftretens der Typvariablen angegeben. Eine Funktion mit der Signatur \texttt{g :: a -> b} instanziiert man
...
Bei Polymorphismus ist zwischen Ad-Hoc Polymorphismus und parametrischem Poly\-mor\-phis\-mus zu unterscheiden. Bei Typvariablen
in Haskell spricht man von parametrischem Polymorphismus, das heißt die entsprechende Funktion arbeitet für sämtliche
...
werden kann, welche der Funktionen jeweils angewandt werden muss. Ein Beispiel für Ad-Hoc-Polymorphismus bietet die
Programmiersprache \textit{C++}: Hier ist es möglich, mehrere Funktionen mit gleichem Namen in einer Klasse zu deklarieren,
solange sie unterschiedliche Signaturen besitzen, wie das folgende Beispiel zeigt.
...
Hier wurden drei Methoden deklariert, die alle den Namen \texttt{log} haben, sich jedoch durch den Typ des Parameters
unterscheiden. Auch Haskell bietet mit sogenannten \texttt{Typklassen} Ad-Hoc-Polymorphismus. Darauf wird später noch genauer
eingegangen.
...
Man kann sich an dieser Stelle die Frage stellen: Wie kann die Funktion die Ergebnisliste erzeugen? Sie kennt den konkreten Typ nicht,
das heißt sie kann keine neuen Elemente ``erfinden''.
...
Um Typen als Relationen aufzufassen, definiert man relationale Aktionen, mit denen zu jedem Typ eine entsprechende Relation
...
die entsprechende relationale Aktion anwendet. Die einzelnen Relationen werden folgendermaßen konstruiert.
...
\cite{reynolds}, ist nun das Parametrizitätstheorem.
...
Das Lifting von Funktionen in den Listenkontext ist ganz einfach das Mapping dieser Funktionen auf Listen, sprich: die Haskell-Funktion
