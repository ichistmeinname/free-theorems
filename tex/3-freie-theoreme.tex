...
Ein Beispiel für Ad-Hoc-Polymorphismus bietet die Programmiersprache \textit{C++} \todo{wahrscheinlich noch was ziteren}: Hier ist es möglich, mehrere Funktionen mit gleichem
...
%\texttt{map\ f} wandelt
%also gewissermaßen die Eingabefunktion \texttt{f} um in eine Funktion, die dieselbe Funktionalität für Listen bietet. Man kann auch sagen:
%Sie \textit{liftet} eine Funktion in den Listen-Kontext.
...
konstruiert werden kann. Johann et al. definieren relationale Aktionen wie folgt \cite{johann2006} \todo{johann et al?}.
...
jeweiligen Typen instanziiert werden. \todo{vielleicht noch bisschen verständlicher}
...
abgerollt wird: \todo{aligns noch ein bisschen verschönern}
...
(vgl. Abschnitt \ref{sec:free-theorems-param} \todo{Richtiger Abschnitt?}). Tatsächlich ist die bereits eingeführte
...
die Typkonstruktorvariable die Funktion $\mathcal{F}$ einsetzt. \todo{viellt noch bisschen verdeutlichen}
...
Aussage ist aber relativ simpel und darüber hinaus kaum weltbewegend. \todo{naja, ob man das so schreiben sollte...}
...
Kmett \todo{ich nehme mal an der heißt wirklich so?}in einem Artikel für \cite{schoolofhaskell.com} \cite{fmap} einen Ansatz, wie man mithilfe des freien Theorems beweisen
kann, dass das zweite Gesetz in Haskell immer gelten muss, wenn das erste Gesetz gilt. \todo{nicht sicher, ob man das so zitieren würde}
...
Im vorangegangenen Schritt wurde bereits erläutert, dass man bei der Spezialisierung zu einigen gelifteten Datentypen
...
(durch die Komposition mit $return_{\kappa}^{-1}$ und $Id$ können diese Elemente nur von dieser Form sein \todo{evtl ausführlicher}).
...
Durch das erste Monadengesetz, \texttt{return a >>= k = k a} \todo{nicht trennen}, gilt die folgende Aussage.
