...
% Eine Relation setzt zwei Mengen in Beziehung. Ein Element aus der ersten Menge kann mit einem Element aus der zweiten
% Menge verwandt sein bezüglich der Relation. Die Relation ist eine Menge von Paaren. Jedes Paar von Elementen bedeutet,
% dass diese in Relation stehen.
% Bezeichnet $\mathcal{R}$ eine Relation, die Elemente aus $T_1$ und $T_2'$ in Beziehung setzt, so schreibt man
% $\mathcal{R} : T_1 \Leftrightarrow T_2$ um auszudrücken, dass $\mathcal{R}$ eine solche Relation ist. Es gilt

% \begin{align*}
% \mathcal{R} \subset T_1 \times T_2
% \end{align*}

Ein wichtiges Werkzeug zur Generierung freier Theoreme sind Relationen. Sie werden verwendet, um Ausdrücke einander
zuzuordnen, die durch Polymorphie ``austauschbar'' sind.
...
Andersherum bedeutet das, dass es Relationen gibt, die man auch als Funktionen auffassen kann. Hat man beispielsweise
...
%Relationen und Funktionen stehen in einer besonderen Beziehung: Man kann jede Funktion als Relation auffassen, wohingegen
%nicht jede Relation auch eine Funktion ist. Funktionen weisen jedem Wert der Ursprungsmenge genau einen Wert der Zielmenge
%zu. Relationen können zu einem Element der Ursprungsmenge keine oder auch mehrere Zuweisungen enthalten.

%Ist eine Funktion $f : T_1 \rightarrow T_2$ gegeben, so kann man diese auch betrachten als Relation, die gegeben ist wie folgt:
...
Beim Ausdruck \texttt{Test} handelt es sich in diesem Fall also nicht um einen Typen, sondern eher um eine Art Funktion, die
einen Typen als Eingabe erwartet und einen Typen zurückgibt. Man spricht hier auch von Sorten (engl. ``kinds''). Haskell
führt für Sorten eine eigene Notation ein. Alle Basistypen wie \texttt{Int}, \texttt{String}, etc. sind von der Sorte *.
Typkonstruktoren, die einen Typen als Parameter erwarten und daraus einen neuen Typen konstruieren, sind vo der Sorte
$* \rightarrow *$.
