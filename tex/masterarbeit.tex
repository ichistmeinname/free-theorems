...
\Leftrightarrow &
\forall k1, k2 \in (* \rightarrow *), \mathcal{K} : k_1 \Leftrightarrow k_2 \\
&\forall A, A' \in Types, \mathcal{A} : A \Leftrightarrow A' \\
&\forall B, B' \in Types, \mathcal{B} : B \Leftrightarrow B' \\
...
&\ \ \ \ \ \ map'\ b\ (fmap_{k1 A B}\ g_1\ y) = fmap_{k2 A' B'}\ g_2\ (map'\ a\ y)
\end{align*}

$map'$ ist hierbei die natürliche map-Funktion, die zu Beginn des Abschnitts bereits erwähnt wurde.
...
Der Punktoperator ist hierbei die Funktionsverkettung. Nun muss man noch zeigen dass
