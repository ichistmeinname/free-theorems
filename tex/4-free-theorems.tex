...
Mal per Hand durchzuführen, da immer die gleichen Schritte durchgeführt werden. Von daher ist es wünschenswert, diesen
...
bereits als Paket verfügbar ist \cite{haskellsrcexts} (bzw. \texttt{haskell-src} als Version, die keine
...
unterstüzt. Der Parser liefert eine eigene Datenstruktur für einen abstrakten Syntaxbaum, aber der
...
hauptsächlich Typsignaturen eine Rolle. Die konkreten Implementierungen der Funktionen, die in einem typischen Programm den Großteil des Programms
...
Listing \ref{lst:ast} zeigt die Datenstruktur am Beispiel \texttt{id :: a $\rightarrow$ a}. Die \texttt{parse}-Funktion des Parser-Moduls von \textit{free-theorems} liefert eine Liste sämtlicher relevanter Definitionen als \texttt{BasicSyntax} zurück.
...
\texttt{Signature}-Konstruktor, der aus dem Namen der Signatur sowie einem Typausdruck besteht.

In diesem Fall ist der Typausdruck eine Funktion zwischen zwei Typvariablen, der Konstruktor \texttt{TypeFun} wird also
...
Hierzu gehören sowohl die \texttt{data}- als auch \texttt{type}- und \texttt{newtype}-Deklarationen. Listing \ref{lst:ast-data} zeigt ein Beispiel für eine
...
der für einen Datentyp den Namen, die übergebenen Typvariablen und die unterschiedlichen Konstruktoren übergeben bekommt, letztere jeweils mit Namen
...
Für die bisher eingeführte Generierung freier Theoreme würde ein solches Striktheitsflag natürlich keine Rolle spielen, da immer nur
die eigentlichen Typen von Bedeutung sind. In Kapitel \ref{sec:striktheit-und-rekursion} wird jedoch kurz darauf eingegangen,
warum Striktheit dennoch eine Rolle bei freien Theoremen spielen kann.
...
demselben Namen \texttt{calcValues} gibt.

Lokale Überprüfungen reichen hier nicht aus, da man innerhalb einer Deklaration
...
Die Prüfungen arbeiten dabei in der \texttt{Writer}-Monade, in der auftretende Fehlermeldungen protokolliert werden.
...
\texttt{RelFunLab} spielt nur für Ungleichungs-Theoreme eine Rolle und wird in dieser Arbeit nicht beachtet.
%wird aber
...
Wie vorangehend erwähnt, werden sämtliche freie Variablen in Typsignaturen geschlossen, was dazu führt, dass \texttt{interpret}
...
Wenn es sich hierbei um eine Funktion handelt, dann wird überprüft, ob es sich beim Typ
um Listen oder um den spezifischen Typ \texttt{Maybe} handelt. In diesen Fällen wird der Ausdruck ersetzt durch die
...
& \hphantom{\forall~ \forall} (test_{T1}\ x, test_{T2}\ y) \in [\mathcal{R}]
\end{align*}
...
& \hphantom{\forall} \forall x \in T_1 \\
& \hphantom{\forall} lift_{[]}(f) (test_{T_1}\ x) = test_{T2} (f\ x)
\end{align*}
...
Handelt es sich um den Typ \texttt{Maybe}, lässt sich stattdessen $fmap\ f$ verwenden.
...
dar. Das Vorgehen ist auch hier sehr generisch, sodass dies von einem Programm leicht durchgeführt werden kann. Betrachten wir
...
Typvariable \texttt{a} als Parameter übergeben. Im freien Theorem wird die relationale Repräsentation eines gelifteten
Datentyps immer durch eine Funktion \textit{lift} dargestellt (auch für Relationen -- das unterscheidet die Bibliothek vom bisher
eingeführten Verfahren). \textit{free-theorems} bietet die Funktion \texttt{unfoldLifts},
...
dass \textit{free-theorems} keine Fehlerprüfungen im Implementierungsteil durchführt. Die Bibliothek kann ohne Auftreten von Fehlern
durchlaufen, selbst wenn der zu einer Funktionssignatur gehörige Code Fehler enthält.
...
dass es sich bei den Tupeln \texttt{("[t1]", "[t2]")} nicht wirklich um Tupel von Zeichenketten handelt, tatsächlich handelt es sich
um Ausdrücke des Typs \texttt{TypeExpression} der \texttt{BasicSyntax}-Struktur. Der Einfachheit halber werden diese hier jedoch als Zeichenketten
...
den Funktionsnamen, für den die Typsignatur überhaupt angegeben wird. Zudem ist angegeben, welche \textit{Teilsprache}
verwendet wird. In diesem Fall ist dies \texttt{BasicSubset}; dieser Wert hängt davon ab, welcher Parameter der \texttt{interpret}-Funktion übergeben wurde.
